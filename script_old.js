                ...
                if(devsRecord[i] !== devsRecord[myId]){
                    for(let ix = 1; ix < dev.length; ix++){
                        if(dev[ix].skill === skillToSearch ){
                            ...
                            //     if (!found) {
                            //     console.log('Mail non inclusa');
                            //     resultsSearch.push([
                            //         { mail: dev[0].mail, surname: dev[0].surname },
                            //         { skill: skillToSearch, years, desc }
                            //     ]);
                            // } else {
                            //     console.log('Mail inclusa');
                            ...
                            //   if (!resultsSearch.some(result => result.mail === dev[0].mail)) {
                            //     console.log('Mail non inclusa');
                            //     resultsSearch.push({ mail: dev[0].mail, skills: [skillToSearch] });
                            //   } else {
                            //     console.log('Mail inclusa');
                            //     let index = resultsSearch.findIndex(result => result.mail === dev[0].mail);
                            //     resultsSearch[index].skills.push(skillToSearch);
                            //   }
    ...
    if(resultsSearch.length === 0){
        ...
        for(let i = 0; i < resultsSearch.length; i++){
            ...
            if(resultsSearch[i] !== myId ){
                ...
                devCard.id = 'developer';
                devCard.innerHTML = resultsSearch[i][0].mail ;
                devCard.innerHTML = `<div class="flex flex-column m-2">
                    <div>${resultsSearch[i][0].mail}</div>
                    <div>${resultsSearch[i][1].skill} ${resultsSearch[i][1].years} ${resultsSearch[i][1].desc}</div>
                    <div>${resultsSearch[i][2].skill} ${resultsSearch[i][2].years} ${resultsSearch[i][2].desc}</div>
                </div>`;
